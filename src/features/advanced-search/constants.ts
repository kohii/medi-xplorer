...
function normalizeMultiValues(values: string[]) {
  const migratedValues = values
    .flatMap((value) => splitByWhitespace(value))
    .map((value) => value.trim())
    .filter(Boolean);
  return [...new Set(migratedValues)];
}
...
export const advancedSearchOperatorOptions = [
  ...
  },
  {
    ...
    negative: false,
    toFieldFilterItem(item: AdvancedSearchItem) {
      const values = item.restValues ? [item.value, ...item.restValues] : [item.value];
      // migrate spaces to commas
      const value = normalizeMultiValues(values).join(",");
      return {
        fieldKey: item.field,
        operator: ":",
        value,
...
export const advancedSearchItemOperatorKinds = advancedSearchOperatorOptions.map(
  (option) => option.kind,
);
