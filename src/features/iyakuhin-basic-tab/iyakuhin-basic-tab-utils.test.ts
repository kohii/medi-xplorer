...
describe("iyakuhin-basic-tab-utils", () => {
  ...
  test("related rows", () => {
    ...
    assert.deepEqual(getRelatedSelectionRows(currentRow, rows, getRowByCode), [selectedRow]);
    assert.deepEqual(getRelatedUnifiedNameRow(currentRow, getRowByCode), unifiedRow);
    ...
    assert.deepEqual(getRelatedSelectionRows(selectionByRelatedDrug, rows, getRowByCode), [
      unifiedRow,
    ]);
  });
